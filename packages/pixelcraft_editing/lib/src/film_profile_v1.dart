...
const filmProfileParameterSpecs = <FilmProfileParameterSpec>[
  FilmProfileParameterSpec(id: 'exposure', label: 'Exposure', min: -2, max: 2, neutral: 0, group: 'Tone', unit: 'EV'),
  FilmProfileParameterSpec(id: 'brightness', label: 'Brightness', min: 0, max: 2, neutral: 1, group: 'Tone'),
  FilmProfileParameterSpec(id: 'contrast', label: 'Contrast', min: 0, max: 2, neutral: 1, group: 'Tone'),
  FilmProfileParameterSpec(id: 'highlights', label: 'Highlights', min: -1, max: 1, neutral: 0, group: 'Tone'),
  FilmProfileParameterSpec(id: 'shadows', label: 'Shadows', min: -1, max: 1, neutral: 0, group: 'Tone'),
  FilmProfileParameterSpec(id: 'saturation', label: 'Saturation', min: 0, max: 2, neutral: 1, group: 'Color'),
  FilmProfileParameterSpec(id: 'temperature', label: 'Temperature', min: -1, max: 1, neutral: 0, group: 'Color'),
  FilmProfileParameterSpec(id: 'tint', label: 'Tint', min: -1, max: 1, neutral: 0, group: 'Color'),
  FilmProfileParameterSpec(id: 'vibrance', label: 'Vibrance', min: -1, max: 1, neutral: 0, group: 'Color'),
  FilmProfileParameterSpec(id: 'vignette', label: 'Vignette', min: -1, max: 1, neutral: 0, group: 'Texture'),
  FilmProfileParameterSpec(id: 'grain', label: 'Grain', min: 0, max: 1, neutral: 0, group: 'Texture'),
  FilmProfileParameterSpec(id: 'sharpen', label: 'Sharpness', min: 0, max: 2, neutral: 0, group: 'Texture'),
  FilmProfileParameterSpec(id: 'gaussian_blur', label: 'Gaussian Blur', min: 0, max: 2, neutral: 0, group: 'Texture'),
  FilmProfileParameterSpec(id: 'curve_shadows', label: 'Curve Shadows', min: -1, max: 1, neutral: 0, group: 'Curve'),
  FilmProfileParameterSpec(id: 'curve_midtones', label: 'Curve Midtones', min: -1, max: 1, neutral: 0, group: 'Curve'),
  FilmProfileParameterSpec(id: 'curve_highlights', label: 'Curve Highlights', min: -1, max: 1, neutral: 0, group: 'Curve'),
  ..._hslSpecs,
];
...
class FilmProfileV1 {
  ...
  static FilmProfileV1 decode(String source) {
    ...
    if (raw is! Map<String, dynamic>) throw const FormatException('Film Profile must be a JSON object');
    if (raw['schema'] != pixelCraftProfileSchema) throw const FormatException('Unsupported Film Profile schema');
    final version = raw['schemaVersion'];
    if (version != pixelCraftProfileSchemaVersion) throw FormatException('Unsupported Film Profile schemaVersion: $version');
    final minEngine = raw['minEngineVersion'];
    if (minEngine is! int || minEngine > pixelCraftProfileEngineVersion) throw FormatException('Film Profile requires engine version $minEngine');
    final id = raw['id'];
    final name = raw['name'];
    ...
    if (rawParameters is Map) {
      for (final entry in rawParameters.entries) {
        if (entry.key is String && entry.value is num) {
          ...
          if (spec == null) throw FormatException('Unsupported Film Profile parameter: $key');
  ...
  static Map<String, double> _normalizeParameters(Map<String, double> input) {
    ...
    for (final entry in input.entries) {
      ...
      if ((value - spec.neutral).abs() > 0.000001) result[entry.key] = value;
    }
    return result;
  }
}
...
class FilmProfileMapping {
  const FilmProfileMapping({required this.sourceField, required this.kind, this.targetField, this.note = ''});
...
FilmProfileImportReport importRecipeMap(Map<String, Object?> recipe, {required String id, String? name}) {
  const exactAliases = <String, String>{
    'exposure': 'exposure', 'contrast': 'contrast', 'highlights': 'highlights', 'shadows': 'shadows',
    'saturation': 'saturation', 'color': 'saturation', 'temperature': 'temperature', 'tint': 'tint',
    'vibrance': 'vibrance', 'vignette': 'vignette', 'grain': 'grain', 'sharpness': 'sharpen', 'sharpen': 'sharpen',
  };
  const approximateAliases = <String, String>{
    'highlightTone': 'highlights', 'shadowTone': 'shadows', 'colorDensity': 'vibrance',
    'wbShiftRed': 'temperature', 'wbShiftBlue': 'temperature',
  };
