...
import 'package:pixelcraft/core/film_profile_recipe.dart';
...
void main() {
  ...
  test('recipe importer reports exact approximate and unsupported fields', () {
    ...
    expect(
      report.mappings.any(
        (mapping) => mapping.sourceField == 'exposure' && mapping.kind == FilmProfileMappingKind.exact,
      ),
      isTrue,
    );
    expect(
      report.mappings.any(
        (mapping) => mapping.sourceField == 'highlightTone' && mapping.kind == FilmProfileMappingKind.approximated,
      ),
      isTrue,
    );
    expect(
      report.mappings.any(
        (mapping) => mapping.sourceField == 'mysteryVendorField' && mapping.kind == FilmProfileMappingKind.unsupported,
      ),
      isTrue,
    );
  });

  test('Film Profile materializes into active draft and truncates redo tail', () {
    ...
    expect(
      operations.any(
        (operation) => operation['type'] == 'filter' && operation['name'] == 'saturation',
      ),
      isFalse,
    );
    expect(
      operations.where(
        (operation) => operation['type'] == 'filter' && operation['name'] == 'contrast',
      ).single['value'],
      0.9,
    );
    expect(
      operations.any(
        (operation) => operation['type'] == 'film_profile' && operation['id'] == 'velvia_inspired',
      ),
      isTrue,
    );
    expect(
      operations.any(
        (operation) => operation['type'] == 'filter' && operation['name'] == 'temperature',
      ),
      isTrue,
    );
  });
